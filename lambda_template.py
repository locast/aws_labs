...
def lambda_handler(event, context):
    ...
    if target_tag_key in instance_tags:
        if target_tag_value == instance_tags[target_tag_key]:
            #update_inventory(instance_tags, instance_ip, instance_user, instance_sshkey)
            ...
            try:
                ...
                s3.download_file(bucket_name, bucket_key, '/tmp/inventory.inv')
                #open the file from S3
                inventory_found = True
                with open('/tmp/inventory.inv','r') as inventory_file:
        ...
        ##### DirecT IN S3 1 GO ###################################
                ...
                with open('/tmp/inventory.inv','w') as inventory_file:
                    ...
                    """
                    if debug:
                        logger.info('final inventory: ' + str(inventory))
                    """
